...
function newPage() {
    ...
    for (let file of customFolder.subTree) if (file.name.includes(newFileName)) newFileIndex++; // TODO: More robust version naming method
...
function resetLocalStorage() {
    if (
        confirm(
            "Do you really want to reset the system?  This will delete all custom files and reset the site to its default state!"
        )
    ) {
        localStorage.clear();
...
function clearMenuDrops() {
    for (let elem of document.getElementsByClassName(styles.menuDropdown)) elem.style.display = "none";
}
                        ...
                        {menu.items.map((item, index) => {
                            ...
                            if (item.itemType === "action")
                                return (
                                    <button
                                        key={index}
                                        id={menu.name.toLowerCase().replace(/" "/g, "") + "Action"}
                                        className={className}
                                        onClick={() => {
                                ...
                                return (
                                    <button
                                        key={index}
                                        className={`w3-button ${styles.menuDropItem}`}
                                        onClick={() => {
...
function StatusFooter({ currentPath, pageName }) {
    ...
    return (
        <footer className={`w3-row ${styles.infoBar}`}>
            ...
            <div id="encodingStatus" className={`w3-col ${styles.infoBarItem} ${styles.encodingStatus}`}>
                UTF-8
            </div>
            <div id="linesStatus" className={`w3-col ${styles.infoBarItem}`}>
                {pStats.lines + " Lines"}
            </div>
            <div id="sizeStatus" className={`w3-col ${styles.infoBarItem}`}>
                {pStats.size <= 1024 ? pStats.size + "B" : Math.round(pStats.size / 102.4) / 10 + "kB"}
            </div>
            <div id="itemStatus" className={`w3-col ${styles.infoBarItem}`}>
                {pageName + ".html"}
            </div>
        </footer>
    );
