            ...
            synopsis={
                <>
                    ...
                    <p>
                        ...
                        <FootRef idx={5} />. Finally, tool-AIs are designed to work more like traditional software in
                        that they do not exhibit goal-directed behavior. The creation of such a system may involve
                        significant amounts of manual tuning and parameter search. Traditional software does not
                        (currently) attempt to form a singleton, so this appears to be a safe option. However, these
                        tools either become difficult to scale
                    ...
                    <p>
                        ...
                        those values be reliably impressed upon it? The book presents several methods, the most
                        promising of which seem to be motivational scaffolding, value learning, and institutional
                        design. Motivational scaffolding proposes that we first create a smaller, more controllable AI
                        which we can train to hold human values. Next, the capabilities of this system are increased to
                        that of a superintelligence. At this point, the agent will be much more resistant to goal
                        change, "locking in" the original goals. Value learning proposes that we train an AI to "do what
                        we would want an AI to do in this situation". This type of adjacent reasoning may be difficult
                        to reliably implement, but it would free programmers from the burden of choosing a specific goal
                        for the model to optimize towards. Final, Bostrom considers designing an institution that would
                        govern the behavior of many AI agents. The main architecture discussed is one composed of{" "}
                        <Latex>{`$n$`}</Latex> superintelligent agents with <Latex>{`$n/2$`}</Latex> less intelligent
                        agents designed to monitor them. This pattern repeats until we are left with one or more humans
                        at the top governing the least capable level of agents.
                    </p>
                    <p>
            ...
            thoughts={
                <>
                    ...
                    <p>
                        ...
                        <FootRef idx={8} /> as it could be at protecting the United States. His <i>orthogonality</i>{" "}
                        thesis is non-trivial when examples of "smart" AI with "dumb" goals are less prevalent in media
                        and (until recently) literature.
                    </p>
                    <p>
        ...
        <BookTile
            ...
            synopsis={
                <>
                    ...
                    <FootRef idx={1} />, developing an intelligence using our current techniques could be catastrophic.
                    More specifically, he asserts that the single-goal optimization objectives used to train modern AI
                    systems could lead to dangerous and unforeseen consequences. AI trained in this manner would likely
                    not reflect the values that we intended to train into it; the failure modes for this system would be
                    dangerously unpredictable. To structure his argument, Russell splits the book into three parts
                    ...
                    <p>
                        ...
                        technology. AI systems do not always behave as you train them, their actions can appear to
                        optimize for the desired objective during training, while they are really optimizing for
                        something else
                    ...
                    <p>
                        ...
                        <FootRef idx={4} />. <i>The King Midas Problem</i> threatens that advanced AI is likely to give
                        us exactly what we ask for, <i>us exactly what we ask for</i>. Just as King Midas' wish turned
                        his food and drink to inedible gold, so could our own wishes lead to unforeseen consequences. As
                        Nick Bostrom discusses at length in <i>Superintelligence</i>, machines designed without
                        sufficient planning are likely to suffer from <i>value alignment</i>. Their
                        values/goals/objectives may be exactly what we ask for, but not exactly what we mean. In
                        ...
                        hateful content
                        <FootRef idx={5} />. <i>Unaligned Instrumental Goals</i> pose another problem. Even if the
                        terminal, or final, goals of a machine are somehow aligned with our intentions, that does not
                        guarantee that instrumental or intermediate goals will be. You can't achieve your terminal goals
                        if you wre dead, so self-preservation becomes a strong instrumental motivator, even of not
                        explicitly trained into a model
                        <FootRef idx={6} />. Compounding these issues is that of an <i>Intelligence Explosion</i>. It
                        would potentially be easier to solve the above problems if we could ensure that the intelligence
                        of artificial minds always grew smoothly and linearly with time. We could carefully craft
                        machines up until just before they become too clever for us to reliably control without having
                        to face the full reality of an existential risk. This is not likely to be the case. With or
                        without human encouragement, the increase of one's intelligence is an instrumental goal
                        applicable to nearly every unbounded or underspecified task. Systems <i>less</i> intelligent
                    ...
                    <p>
                        ...
                        <FootRef idx={7} />? Researchers in the field of AI may also deflect sound proposals for
                        alignment strategies or preparation, while acknowledging their technical validity. Likely the
                        most straightforward warnings given by AI safety researchers is to simply slow down research.
                        Many scientists may agree with this in principle, but their employment likely hinges on doing
                        the precise opposite of this. Another common argument Russel observes disengages with the topic
                        of AI safety by introducing a new topic that should be attended to instead. Oftentimes,
                    ...
                    <p>
                        ...
                        people
                        <FootRef idx={9} />. Additionally, how much weight would a utility-maximizing robot assign to
                        its owner over others. At either end of this scale, you either get selfish robots all over again
                        or robots so altruistic that there is no personal advantage to buying one at all. Finding a
                        balance that makes this idea practically workable will be difficult.
                    </p>
                    <p>
                        ...
                        change in our goals, but are rather consequences of our limitations. We may not see a clear
                        avenue to our goals and may act erratically from the perspective of a rational agent. It is
                        important for robots to be able to model and predict these states without directly experiencing
                        them internally.
                    </p>
                </>
            }
        ...
        <BookTile
            ...
            synopsis={
                <>
                    ...
                    work, Hofstadter attempted to explore his ideas on Souls, Selves and <i>I's</i> through the lense of{" "}
                    <i>Gödel, Escher, Bach</i>'s three namesakes. The "strange" recursive and self-referential natures
                    of their work served as an anchor for the author to ground his own ideas on the human mind in. By
                    contrast, <i>I Am a Strange Loop</i> uses these references less as the core of his thesis and more
                    as supporting elements. Instead, he uses less abstract examples to present his ideas to the reader.{" "}
                    ...
                    <p>
                        ...
                        <FootRef idx={1} />. According to the book, larger souled beings (measured in <i>Huneker</i>{" "}
                        units), in roughly increasing order, include bees, fish, chickens, dogs, infants, adults with
                        severe cognitive impairments, and finally healthy adult humans. Notably, this list is not based
                        simply on an entity's similarity to humans
                        <FootRef idx={2} />. Instead, the size of an entity's soul is correlated only to its cognitive
                        abilities for theory of mind, self-referential thought, and similar abilities. This measurement
                        of a soul is also completely divorced from the substrate for cognition. Fictional robots, like{" "}
                        <i>C-3PO</i> and <i>R2-D2</i> are only made of seemingly inert metal, yet they appear to have
                        souls comparable to humans. How can this be?
                    </p>
                    <p>
                        ...
                        composed of organelles, which are made up by various proteins and molecules, which themselves
                        are made of atoms, then sub-atomic particles. You would be hard-pressed to find someone claiming
                        that quarks or strings or hypergraphs are themselves conscious
                        <FootRef idx={4} />, so how can we, conscious <i>I's</i>, ourselves be conscious? The key,
                        Hofstadter argues, is not in the material, it lies in patterns of interaction. To better
                        illustrate this thesis, the text introduces Small Interacting Magnetic Marbles (<i>Simms</i>).
                        these Simms are constantly in motion, bouncing around within the billiard table-like{" "}
                        <i>Careenium</i>. Suppose that the walls of this table occasionally flex inward, changing the
                        trajectory of Simms as they bounce off of it. Suppose further that these magnetic marbles can
                        fuse together in clumps or fission into components, depending on how other Simms interact with
                        them. From the perspective of a simm, these <i>Simmballs</i> appear to be massive, immovable
                        conglomerates of Simms. Zooming out and speeding up, the Simms themselves disappear into a
                        background blur anf the much larger Simmballs begin to noticeably move. Now, we only see the
                        Simmballs as they interact with each other, fission apart, fuse, and evolve into complex
                    ...
                    <p>
                        ...
                        first place. Feedback and self-reapplication is found at the core of exponential functions, the
                        output of the last multiplication is fed back into subsequent multiplications. In a concrete
                        sense, the familiar screech of audio feedback is the result of an amplified sound being fed back
                        into the amplifier through the microphone. Video feedback is somewhat different, however. In the
                        case of a simple camera pointing at a screen, very little amplification is present. Instead of
                        an explosion of intensity, the recursion of images converges on a single, seemingly infinitely
                        far vanishing point. Here, the original image is faithfully recreated within successor images in
                        a basic form of self-reference. This self-reference was a major concern among early twentieth
                        century mathematicians. Near the turn of the century, Bertrand Russell had been working on
                        grounding all of mathematics inside a basic theory of sets. Self-reference plagued this early
                        attempt, leading to contradictions at the core of his framework. <i>Russell's paradox</i>, as it
                        became known, concerns the set of all sets, namely, does it contain itself? A system that
                        allowed this paradox quickly became unworkable, therefore Russell and his colleague Alfred
                        Whitehead sought to purge it from their theories. Their new approach, published in{" "}
                        ...
                        could only be defined, or reference, objects in lower strata. Their <i>magnum opus</i> was
                        intended to be a fortress, impregnable by inconsistencies or incompleteness. Unfortunately, even
                        the most hardened strongholds still contain innumerable, microscopic cracks.
                    </p>
                    <p>
                        ...
                        information. They take in stimuli and echo it back after some form of transformation that is
                        only a function of the initial input. More complex systems may gain the ability to perceive.
                        Incoming information may be stored and later synthesized into more complex transformations on
                        later information. These auxiliary symbols function as <i>symbols</i> that are dormant until
                        selectively triggered by other symbols. As an example, Hofstadter describes mosquitoes as simple
                        feedback mechanisms without access to higher-level symbols. These creatures have no need to
                        differentiate between different types of animals or threats. To survive, it must only know to
                        fly towards warm, flesh-like materials and to fly away from anything rapidly approaching their
                        position that may pose a threat. As a consequence of this, Hofstadter argues that they do not
                        have any capacity for self-reference, no "I" symbol to use in reasoning about themselves. Moving
                        up in <i>Huneker</i> units, the text considers the dog. Through still below humans in terms of
                        cognitive capacity, the imprint of an <i>I</i> begins to emerge. Dogs have a limited scope of
                        symbols to draw from: their owner(s), their favorite toy, their least-favorite dog, or the
                        postal worker. Each symbol is activated by a different set of other symbols and stimuli.
                        Notably, the symbol repertoire of a dog has limited extensibility: most can only learn a few
                        dozen commands and tricks before reaching their cognitive capacity. Humans, Hofstadter claims,
                        have an <i>arbitrarily</i> extensible set of symbols. We can define a near limitless number of
                        concepts in terms of existing symbols and learned experiences. This generality is the missing
                        component when comparing humans to creatures of "lesses" souls.
                    </p>
                </>
            }
            thoughts={<></>}
