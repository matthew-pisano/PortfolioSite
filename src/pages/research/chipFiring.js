...
export default function ChipFiring() {
    ...
    let tiles = [
        ...
        new Tile(
            ...
            `In the study of this game a <u>divisor</u> is an integer vector $v\\in\\mathbb{Z}^n$
                where <i>n</i> is the number of vertices in the graph.  The $i^{th}$ of element of the vector, 
                <i>v</i>, is the number of chips on the $i^{th}$ vertex of the graph.  Two divisors have 
                an <u>equivalence relation</u> (~) if one divisor can be gotten from the other by a
                finite series of lending or borrowing moves 
                $D_1 \\sim D_2 \\xleftrightarrow{} (D_1 \\xleftrightarrow{\\text{moves}} D_2)$.
                An <u>equivalence class</u> <i>[D]</i> is the set of all divisors that are equivalent to 
                each other, $[D] = \\{D_i~|~D_i \\sim D\\}$.`,
            "", [], [], "", true
        ),
        new Tile(
            "The Picard Group and The Jacobian",
            `The <u>Picard Group</u> of a graph is the set of all equivalence classes that the 
                divisors of that graph can be a part of.  The <u>Jacobian</u> of a graph $Jac(G)$ is 
                a subset of $Pic(G)$ such that every divisor in each equivalence class has a degree 
                of $0$ where the degree of a divisor $deg(D)$ is the sum of each of the divisor's elements.  
                If a divisor is in one of the Jacobian's classes, it can be made winning after a 
                finite series of moves.  The Picard group of a graph is often written in the structure 
                of $Pic(G) = Jac(G) \\times \\mathbb{Z}^n$ where $n$ is the rank of the Picard group.
        ...
        new Tile(
            ...
            `The <u>laplacian</u> of a graph, in this case is an <i>n x n</i> matrix where 
                ...
                elements are valued such that $L_{ij}$ is negative the number of edges shared between 
                vertices <i>i</i> and <i>j</i>.  An example for a square graph is $L =
                \\begin{bmatrix}
                2 & -1 & -1 & 0\\\\
                -1 & 2 & 0 & -1\\\\
                -1 & 0 & 2 & -1\\\\
                0 & -1 & -1 & 2
                \\end{bmatrix}$
                or $L = D - A$ where $D$ is the diagonal matrix and $A$ is the adjacency matrix for 
                the graph.  The laplacian of a graph encodes all the valid moves that can be preformed 
        ...
        new Tile(
            ...
            `Tree graphs are one of the simplest structures that we investigate and their 
                ...
                fact that a tree graph only has one spanning tree.<br><br>
        ...
        new Tile(
            ...
            `Cycle graphs are another simple graph that we study, although their Picard groups 
                ...
                the Jacobian is $\\mathbb{Z}_n$ and for one path, it is always trivial.  For two 
                paths, the Jacobian is more complex, but also well-defined.  The Jacobian is 
                $\\mathbb{Z}_{x-2}$, where $x$ is the number of bidirectional paths counter-clockwise 
                from the clockwise path and clockwise from the counter-clockwise path with no yet 
                noted exceptions.  For all other numbers of paths, we have not yet discovered a 
                complete description as we have with two paths.  However, we have noticed that there 
                is also always a single invariant factor and this invariant factor is almost always 
                $\\mathbb{Z}_2 \\dots \\mathbb{Z}_4$ for sizes of graphs smaller than size fifteen.  
                We suspect that, for very large graphs, the behavior of four or more paths approaches 
                that of two paths.<br><br>
        ...
        new Tile(
            ...
            `Wheel graphs are similar to cycles in their construction, but not in their 
                ...
                the Jacobians of wheels of different sizes.  The Jacobian of a wheel graph of size $n$ 
                is equal to $\\mathbb{Z}_{\\alpha\\phi^{n}} \\times \\mathbb{Z}_{5\\alpha\\phi^{n}}$ 
                when the size was odd where $\\alpha \\approxeq 0.27555$ and 
                $\\mathbb{Z}_{\\beta\\phi^{n}} \\times \\mathbb{Z}_{\\beta\\phi^{n}}$
