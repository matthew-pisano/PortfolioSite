...
export default function ImageCrypt() {
    ...
    let tiles = [
        new Tile(
            <h2>Overview</h2>,
            <><i>Image-Crypt</i> encodes any arbitrary byte sequence into a target image file.
                This is performed by adding small perturbations to the pixel values within an image. 
                The noise may optionally be generated based off of a key file to further obfuscate it 
                using a hash-based encoding. The encoded document can be extracted from the image using
                the same key file.</>,
        ...
        new Tile(
            ...
            <>Data is encoded into an image by inserting noise into the image. The noise is
                ...
                channel. The bit width can be set using the <code>-b</code> flag. The following bit 
                widths are available: 1, 2, and 4.  Higher bit widths can encode more data into an 
                image, but they will also make the encoded data more obvious.
                <br/><br/>
                ...
                Next, the bits of each character are encoded into an image's pixels by
                adding <code>[0-bitWidth)</code> to it, depending on the bits within the character to be encoded.
                <br/><br/>
                ...
                value is 100, the character value is <code>M (ASCII-77)</code>, the first two bits of 
                M (<code>1001101</code>) are <code>01</code> (little-endian), so the channel value 
                will be set to 101. The next channel will encode the next two bits
                of <code>1001101</code>: <code>11</code>. If the value of the next channel is 124, its
                new value will be 127. With this bit width, each 4-channel pixel can encode a single, 
                1-byte character as each channel encodes 2-bits within its noise. The noise always 
                stays within the space created by the rounding, so the modulo of the value can be 
                used to losslessly decode the text.
                <br/><br/>
        ...
        new Tile(
            ...
            <>To make the encoded data less obvious, it can be obfuscated using a key file.
                ...
                <br/><br/>
                Plain-text documents that are obfuscated with full-text shifting have had their bits 
                shifted uniformly upward based off of the hash of the key. This helps to make the 
                encoded text less obvious, but it is still possible to decode the text by brute-force 
                guessing the shift value.  The same principle is applied to all arbitrary byte sequences.
                <br/><br/>
                ...
                <br/><br/>
                For example, if encoding the fourth character of <code>hello world</code> using the
                key <code>secret</code>, the character <code>o</code> would be shifted by the <code>hash % 128</code> of
