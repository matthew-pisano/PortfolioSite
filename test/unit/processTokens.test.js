...
describe('Test insertVars', () => {
    ...
    test('Test Valid Multiple Vars', () => {
        expect(insertVars(testEnv, "Equ: $var2+$var = $variable"))
            .toBe(`Equ: ${testEnv["var2"]}+${testEnv["var"]} = ${testEnv["variable"]}`);
        expect(insertVars(testEnv, "Disk:$var$variable"))
            .toBe(`Disk:${testEnv["var"]}${testEnv["variable"]}`);
    });
